...
from modules.common import log, GLOBAL_CONFIG, check_range_overlap, deserialize_json_container, serialize_json_container
...
def main(project_to_analizys: str):
	'''
		...
		The process to find the affected will start for the project of the params.
	...
	for project in project_list:
		...
		input_csv_files, output = project.find_last_diff_cves(project.output_directory_diff_path, project, INPUT, OUTPUT)

		# Iterate for all the inputs ("novas" and "atualizadas")
		for input_csv_path in input_csv_files:
			...
			for topological_index, neutral_commit_hash in enumerate(neutral_commit_list):
				...
				if topological_index % CSV_WRITE_FREQUENCY == 0:
					...
					affected_files.to_csv(output, index=False)
